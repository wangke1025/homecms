    ...
    public function get_images_path($images_path = 'uploads')
    {
       if ('uploads' != $images_path && !preg_match('#^(uploads)/(.*)$#i', $images_path)) {
        ...
        if (1 >= count($arr1)) { // 只有一级目录才显示常用图片
            $common_pic = M('common_pic')->order('id desc')->limit(6)->field('pic_path')->select();
        }
        ...
        if (0 <= $num) {
            ...
            if (!empty($total)) {
                ...
                if (!empty($isExistPic)) {
                    $arr_file[] = [
                        'id'        => $num,
                        'url'       => url('Uploadify/get_images_path',['num'=>$param['num'],'func'=>$param['func'],'images_path'=>$directory]),
                        'target'    => 'content_body',
                        'isParent'  => true,
                        'open'      => $open,
                        'dirpath'   => $directory,
                ...
                else if(($dir != ".") AND ($dir != ".."))
                {
                    ...
                    if (!empty($total)) {
                        ...
                        if (!empty($isExistPic)) {
                            $arr_file[] = [
                                'id'        => $num,
                                'url'       => url('Uploadify/get_images_path',['num'=>$param['num'],'func'=>$param['func'],'images_path'=>"$directory/$dir"]),
                                'target'    => 'content_body',
                                'isParent'  => false,
                                'open'      => false,
